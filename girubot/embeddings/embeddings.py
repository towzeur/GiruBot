...
from girubot.config import QUEUE_MAX_DISPLAYED
from girubot.cogs.music.player_queue import PlayerQueue

from .utils import convert_to_youtube_time_format, Unicode
from .markdown import bold, code
...
def queue(ctx, queue: "PlayerQueue", page=1):
    ...
    # current
        ...
        if queued > 0:
            embed.add_field(name=Unicode.VOID, value="__Up Next:__", inline=False)
...
def ping(api_response_time: int, message_response_time: int, websocket_heartbeat: int):
    """
    API response time
    ...
    embed.add_field(
        name=Unicode.VOID, value=f":hourglass: {api_response_time}ms", inline=False,
    )
    embed.add_field(
        name=Unicode.VOID, value=f":stopwatch: {message_response_time}ms", inline=False,
    )
    embed.add_field(
        name=Unicode.VOID, value=f":heartbeat: {websocket_heartbeat}ms", inline=False,
    )
    return embed
